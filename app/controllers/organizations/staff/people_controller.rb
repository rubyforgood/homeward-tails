class Organizations::Staff::PeopleController < Organizations::BaseController
  ...
  def validate_person_does_not_exist
    if Person.where(user_id: Current.user.id).exists?
  ...
  def validate_inputs
    if person_params[:email] != Current.user.email
      flash.now[:alert] = "Email must match the one you registered with"
      render :new
    end
  end
